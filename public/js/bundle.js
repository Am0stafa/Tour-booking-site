...
var displayMap = function displayMap(locations) {
  ...
  var bounds = new mapboxgl.LngLatBounds(); //^ now we loop over the locations and make a mark above each one
  ...
  locations.forEach(function (loc) {
    ...
    new mapboxgl.Marker({
      element: el,
      anchor: 'bottom'
    }).setLngLat(loc.coordinates).addTo(map); //* here we create a pop up which will display information about the location above the mark and it is a bit similar to creating a marker
    //& Add popup

    new mapboxgl.Popup({
      //^ to fix a bug which is that the pop up over lap th marker to we want to move it up
      offset: 32
    }).setLngLat(loc.coordinates).setHTML("<p>Day ".concat(loc.day, ": ").concat(loc.description, "</p>")).addTo(map); //? the last thing we need to to
